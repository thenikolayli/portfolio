...
class LinkSerializer(serializers.ModelSerializer):
    # checks for duplicate name and banned characters
    def validate_name(self, value):
        if Link.objects.filter(name=value).exists():
            raise serializers.ValidationError("A link with that name already exists")

        for each in settings.BANNED_LINK_CHARACTERS:
            if each in value:
                raise serializers.ValidationError(f"'{each}' not allowed in link name")
        return value
